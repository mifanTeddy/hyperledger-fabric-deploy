...
func setVaccineData(stub shim.ChaincodeStubInterface, args []string) (string, error) {
    if len(args) != 2 {
...
func getVaccineData(stub shim.ChaincodeStubInterface, args []string) (string, error) {
    if len(args) != 1 {
    ...
    if value == nil {
        return "", fmt.Errorf("Vaccine not found: %s", args[0])
    }
    return string(value), nil
}


func getVaccineHistory(stub shim.ChaincodeStubInterface, args []string) (string, error) {
...
func (c *AssertsExchangeCC) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
    ...
    switch funcName {
    case "set":
        result, err = setVaccineData(stub, args)
    case "get":
        result, err = getVaccineData(stub, args)
    case "getHistory":
        result, err = getVaccineHistory(stub, args)
    default:
